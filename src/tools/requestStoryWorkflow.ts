...
const registerRequestTool = (
  ...
  server.registerTool(
    ...
    async ({ workflowId, story, answers, autoExecute, autoVerify, baseBranch, planningAutoAccept }) => {
      ...
      if (wantsExecutionOptions && !executionEnabled) {
        return {
          ...
          structuredContent: {
            ...
            hint:
              "requestStoryWorkflow is planning-only unless COWAI_ENABLE_WORKFLOW_EXECUTION=true. Use claimTask -> submitTask -> acceptTask/rejectTask.",
          },
          isError: true,
        };
      }
      ...
      const allDone = currentWorkflowTasks.length > 0 && currentWorkflowTasks.every((t) => t.status === "done");
      workflow.status = allDone ? "done" : "active";
      workflow.report = `workflow=${workflow.id}; status=${workflow.status}; tasks=${currentWorkflowTasks.length}`;
      workflow.updatedAt = getIsoTime();
...
export const registerRequestStoryWorkflowTool = (server: McpServer) => {
  registerRequestTool(
    server,
    "requestStoryWorkflow",
    "requestStoryWorkflow",
    "PM/planning gateway: receive a request, clarify requirements, and decompose it into backlog tasks.",
  );
};
