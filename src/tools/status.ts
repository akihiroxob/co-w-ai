...
export const registerStatusTool = (server: McpServer) =>
  server.registerTool(
    ...
    {
      title: "status",
      description: "Get current orchestrator status (summary + tasks + workflows + activity + runs).",
      inputSchema: {},
    },
    async () => {
      ...
      const runSummary = Object.values(state.runs).reduce<Record<RunStatus, number>>(
        (acc, r) => {
